...
def calc_neighbor_weightmatrix_for_TV(hp, nside):
    nside_now = nside
    Npix = 12 * nside **2
    Neighbor_matrix = np.zeros((Npix,Npix))
    Weight_tsv_matrix = np.zeros((Npix,Npix))
    ...
    for i in range(Npix):
        neighbor = hp.get_all_neighbours(nside_now, i)
        neighbor_for_arr = neighbor
        neighbor_for_arr[neighbor_for_arr<0] = i
        neighbor_arr.append(neighbor_for_arr)
        ...
        for j in range(8):
            ...
            Weight_tsv_matrix[neighbor_ind][i] -= 0.5
            Weight_tsv_matrix[neighbor_ind][neighbor_ind] += 0.5
    return Weight_tsv_matrix,Neighbor_matrix, np.array(neighbor_arr)
...
@jit
def TV_healpix(neighbor_arr, image):
    arr_n1, arr_n2 = np.shape(neighbor_arr)
    result = 0
    for i in range(arr_n1):
        for j in range(arr_n2):
            if (image[i]-image[neighbor_arr[i][j]]) >0:
                result += (image[i]-image[neighbor_arr[i][j]])
            else:
                result += (-image[i]+image[neighbor_arr[i][j]])

    return result

#def TV_healpix(Neighbor_matrix, image):
#    xv, yv = np.meshgrid(image, image, sparse=False, indexing='ij')    
#    return np.sum(np.abs(xv-yv) * Neighbor_matrix)

## Calculation of d_TSV
def d_TSV_healpix(weight, image):
    return 2 * np.dot(weight,  image)

#def d_TV_healpix(Neighbor_matrix , image, epsilon = 1e-8 ):
#    xv, yv = np.meshgrid(image, image, sparse=False, indexing='ij')
#    dif = xv - yv
#    total = 2 * Neighbor_matrix * dif * (1/( (xv-yv)**2 + epsilon**2) ) 
#    return np.einsum("ij->i", total)
@jit
def d_TV_healpix(neighbor_arr , image, epsilon = 1e-8 ):
    arr_n1, arr_n2 = np.shape(neighbor_arr)
    d_TV = np.zeros(arr_n1)
    for i in range(arr_n1):
        for j in range(arr_n2):
            dif = image[i] - image[neighbor_arr[i][j]]
            d_TV[i] += dif/np.sqrt((dif*dif + epsilon**2))
...
def F_TV_healpix(data, W,  x_d, lambda_tv, weight, sigma):
    data_dif = data -  np.dot(W, x_d)
    return (np.dot(data_dif, (1/sigma**2) *data_dif)/2)  + TV_healpix(weight, x_d) *  lambda_tv
...
def dF_dx_healpix_TV(data, W, x_d, lambda_tv, weight, sigma):
    data_dif = -(data -  np.dot(W, x_d))
    return np.dot(W.T, (1/sigma**2) *data_dif) +lambda_tv*  d_TV_healpix(weight, x_d)
...
def calc_Q_part_healpix_TV(data, W,  x_d2, x_d, df_dx, L, lambda_tv, weight, sigma):
    Q_core = F_TV_healpix(data, W, x_d, lambda_tv, weight, sigma)
    Q_core += np.dot((x_d2 - x_d), df_dx) + 0.5 * L * np.dot(x_d2 - x_d, x_d2 - x_d)
    return Q_core
...
def soft_threshold_healpix(x_d, eta, crit=0.1, crit_flag = False):
    ...
    if crit_flag:
        mask = x_d>crit
        mask2 = x_d < -crit
        vec[mask] = x_d[mask] - crit
        vec[mask2] = x_d[mask2] + crit
    else:
        mask = x_d>eta
        mask2 = x_d < -eta
        vec[mask] = x_d[mask] - eta
        vec[mask2] = x_d[mask2] + eta
    return vec
...
## Function for MFISTA
def mfista_func_healpix(I_init, d, A_ten, weight,sigma, lambda_l1= 1e2, lambda_tsv= 1e-8, L_init= 1, eta=1.02, maxiter= 10000, max_iter2=100, 
...
def mfista_func_healpix_TV(I_init, d, A_ten, weight_TV,sigma, lambda_l1= 1e2, lambda_tv= 1e-8, L_init= 1, eta=1.02, maxiter= 10000, max_iter2=100, 
                    miniter = 2000, TD = 30, eps = 1e-5, log_flag = False, do_nothing_flag = False, prox_map = soft_threshold_nonneg_healpix, prox_crit=0.1, prox_crit_flag=False):
    if do_nothing_flag == True:
        return I_init
    ...
    ## The initial cost function
    cost_first = F_TV_healpix(d, A_ten, I_init, lambda_tv, weight_TV, sigma)
    cost_first += lambda_l1 * np.sum(np.abs(I_init))
    cost_temp, cost_prev = cost_first, cost_first
    cost_min = cost_temp  * 1e10
    ...
    for iter_now in range(maxiter):
        ...
        ##df_dx(y)
        df_dx_now = dF_dx_healpix_TV(d, A_ten, y, lambda_tv, weight_TV, sigma) 
        ...
        for iter_now2 in range(max_iter2):
            ...
            y_now = prox_map(y - (1/L) * df_dx_now, lambda_l1/L,  prox_crit, prox_crit_flag)
            Q_now = calc_Q_part_healpix_TV(d, A_ten, y_now, y, df_dx_now, L,  lambda_tv, weight_TV, sigma)
            F_now = F_TV_healpix(d, A_ten, y_now,lambda_tv,weight_TV, sigma)
        ...
        #logger.debug("iter_now %d, L: %f, F_sum:%f, l1_term:%f" % (iter_now, L, F_now, lambda_l1 * np.sum(np.abs(y_now))))
        if iter_now % print_iter  == 0:
            log_out_now = "l1:%.2f, ltv:%.2f, Current iteration: %d/%d,  L: %f, cost: %f, cost_chiquare:%f, cost_l1:%f, cost_ltv:%f" % (np.log10(lambda_l1), np.log10(lambda_tv), iter_now, maxiter, L, cost_temp, F_obs_healpix(d, A_ten, y, sigma),lambda_l1 * np.sum(np.abs(y)),lambda_tv * TV_healpix(weight_TV,y))
            logger.info(log_out_now)
        if(iter_now>miniter) and cost_arr[iter_now-TD]-cost_arr[iter_now]<cost_arr[iter_now]*eps:
            break

        mu = mu_new
    log_out_now = "l1:%.2f, ltv:%.2f, Current iteration: %d/%d,  L: %f, cost: %f, cost_chiquare:%f, cost_l1:%f, cost_ltv:%f" % (lambda_l1, lambda_tv, iter_now, maxiter, L, cost_temp, F_obs_healpix(d, A_ten, y, sigma),lambda_l1 * np.sum(np.abs(y)),lambda_tv * TV_healpix(weight_TV,y))
    if log_flag:
        logger.debug(log_out_now)
    return x_best

def ComputeContours_for_subplot(P, Levels, file_save, title = "", fontsize = 18, unit_label ="", fig="", subplot=""):

    now_fontsize = matplotlib.rcParams['font.size']
    ...
    hp.visufunc.mollview(P.reshape(-1),flip='geo',cmap='brg', fig = fig, sub = subplot, title=title, 
        unit=unit_label, min = float("%.4f" % np.min(P)), max = float("%.4f" % np.max(P)))
    print (np.min(P))
    hp.graticule()
    
    #for iCont in range(len(Cont_coll)):
    #    hp.visufunc.projplot(Cont_coll[iCont].vertices[:,0],Cont_coll[iCont].vertices[:,1],lonlat=True,color='k')#, sub = subplot)

    matplotlib.rcParams.update({'font.size': now_fontsize })


def cart_sub(P, Levels, file_save, title = "", fontsize = 18, unit_label ="", fig="", subplot="", min_max_0_1=None):
    ...
    #Cont_coll= Cont.collections[0].get_paths()
    if min_max_0_1 == None:
        hp.visufunc.cartview(P.reshape(-1),flip='geo',cmap='brg', fig = fig, sub = subplot, title=title, 
        unit=unit_label, min = float("%.4f" % np.min(P)), max = float("%.4f" % np.max(P)))
    else:
        hp.visufunc.cartview(P.reshape(-1),flip='geo',cmap='brg', fig = fig, sub = subplot, title=title, 
        unit=unit_label, min = 0, max = 1)
    print (np.min(P))
    hp.graticule()
...
def ComputeContours(P, Levels, file_save, title = "", fontsize = 18, unit_label =""):
    ...
    Cont_coll= Cont.collections[0].get_paths()
    hp.visufunc.mollview(P.reshape(-1),flip='geo',cmap='brg', title=title, unit=unit_label)
    hp.graticule()
...
class main_sparse_healpix:
    ...
    def __init__(self, d, A_ten, weight, sigma, heal_py, eta = 1.02, L_init = 1, maxiter= 4000, maxiter2=100, miniter = 2000, TD = 50, eps = 1e-7, prox_map = soft_threshold_nonneg_healpix, crit = 0.1, crit_flag = False):
   ...
   ##  Return "n_fold"ed index given (n_len) data
    ...
    def solve_and_make_figure(self, lambda_l1, lambda_tsv, file_name, _debug =False):
        ...
        ComputeContours(I_est,  [np.percentile(I_est, level_contour)], file_name)

        file_name_hist = file_name.split(".pdf")[0] + "_hist.pdf"

        plt.hist(I_est, bins = 100, log=True)
        plt.title("l1:%.2f, ltsv:%.2f,  #(I_est==0) is %d" % (lambda_l1, lambda_tsv, len(I_est[I_est==0])))
        plt.xlabel("m")
        plt.ylabel("Number")
