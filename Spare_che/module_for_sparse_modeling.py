...
class main_sparse:
    ...
    def cv(self, lambda_l1_row, lambda_tsv_row, n_fold, file_name_head = "cv", folder_name = "./", file_out = "cv_result"):
        ...
        for l1_now in lambda_l1_row:
            for ltsv_now in lambda_tsv_row:
                ...
                self.l1_term.append(l1_term)
                self.chi_result.append(chi_term)
                self.tsv_term.append(tsv_term)


        # Outputting result
        self.cv_result_out(file_out + ".dat")
        plotter = CVPlotter(len(l1_arr), len(ltsv_arr), l1_arr, ltsv_arr)
        plotter.make_cv_figure(folder_d_name, outfile=file_out+".png", file_name_head=file_name_head)
    ...
    def cv_result_out(self, file_out):
        ...
        for i in range(len(self.MSE_result)):
            print ("%e, %e, %e, %e, %e, %e, %e" % (self.MSE_result[i], self.MSE_sigma_result[i],self.l1_result[i], self.ltsv_result[i],self.chi_result[i],self.l1_term[i],self.ltsv_term[i]), file = file_now)
        file_now.close()
    ...
    def without_cv_result_out(self, file_out):
        ...
        for i in range(len(self.MSE_result)):
            print ("%e, %e, %e, %e, %e, %e, %e" % (self.l1_result[i], self.ltsv_result[i],self.chi_result[i],self.l1_term[i],self.ltsv_term[i]), file = file_now)
        file_now.close()
