...
class main_sparse:
    ...
    def cv_result_out(self, file_out):
        ...
        for i in range(self.MSE_result):
            print ("%f, %f, %f, %f" % (self.MSE_result[i], self.MSE_sigma_result[i],self.l1_result[i], self.ltsv_result[i]), file = file_now)
        file_now.close()



    def solve_without_cv(self, lambda_l1_row, lambda_tsv_row, file_name_head = "cv", folder_name = "./", print_status = True, file_out = "cv_result"):
        ...
        for l1_now in lambda_l1_row:
            for ltsv_now in lambda_tsv_row:
                ...
                if print_status:
                    ...
                    print("l1: %d/%d, ltsv: %d/%d. elapsed time: %f" % (np.log10(l1_now), np.log10(np.max(lambda_l1_row)), 
                        np.log10(ltsv_now), np.log10(np.max(lambda_tsv_row)), t2-t1))
                    print ("total cost:%f, chi:%f, l1:%f, ltsv:%f \n" % (chi_term+tsv_term + l1_term, chi_term, l1_term, tsv_term))
